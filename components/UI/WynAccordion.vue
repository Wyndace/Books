<template>
  <article
    class="rounded-3xl bg-blue-500 transition-[border-radius] duration-75 text-white"
    @click="open"
    aria-expanded="false"
  >
    <button class="flex items-center justify-between w-full h-24 rounded-3xl p-6 gap-3 transition-[border-radius] duration-75">
      <h3 class="text-base text-left font-black">{{ title }}</h3>
      <AccordionIcon class="!w-5 !h-5 !mb-0 transition-transform duration-75" />
    </button>
    <p
      class="max-h-0 opacity-0 duration-75 w-full text-left overflow-hidden px-6"
      aria-hidden="true"
    >
      {{ text }}
    </p>
